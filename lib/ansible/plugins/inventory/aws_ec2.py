...
DOCUMENTATION = '''
    ...
    options:
        ...
        filters:
          description:
              - A list of sets of filters to apply. If no filters are specified, all non-terminated instances are returned.
              - Instances matching at least one filter set will be returned.
              - Within a set all filter conditions must be met in order for an instance to match.
              - Available filters are listed at U(http://docs.aws.amazon.com/cli/latest/reference/ec2/describe-instances.html#options).
          type: list
          default: []
...
regions:
  ...
  - us-east-2
filters:
  # All dev and QA hosts in sg-xxxxxxxx
  tag:Environment:
    - dev
    - qa
...
# Example using multiple sets of filters
plugin: aws_ec2
regions:
  - us-east-1
filters:
  # Return all instances in either of these AZs
  - availability-zone:
      - us-east-1a
      - us-east-1b
  # Also return t3.micro instances in 1c
...
class InventoryModule(BaseInventoryPlugin, Constructable, Cacheable):
    ...
    def _query(self, regions, filters, strict_permissions):
        ...
        if filters:
            for filter_set in filters:
                results.extend(self._get_instances_by_region(regions, filter_set, strict_permissions))
        else:
            results = self._get_instances_by_region(regions, filters, strict_permissions)
    ...
    def parse(self, inventory, loader, path, cache=True):
        ...
        filters = [ansible_dict_to_boto3_filter_list(x) for x in raw_filters]
